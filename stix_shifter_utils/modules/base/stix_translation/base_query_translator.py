...
class BaseQueryTranslator(object, metaclass=ABCMeta):
    ...
    def transform_query(self, data):
        ...
        if unmapped_stix:
            ...
            self.logger.warning(f"The following STIX fields are not supported : {set(unmapped_stix_collection)} with dialect {self.dialect}. The request will ignore those fields. This can result in results that do not match the request.")
        if unmapped_operator:
            unmapped_operator_collection.extend(unmapped_operator)
            self.logger.warning(f"The following STIX operators are not supported : {set(unmapped_operator_collection)} with dialect {self.dialect}. The request will ignore those fields. This can result in results that do not match the request.")
        if antlr_parsing:
