...
class TestQueryTranslator(unittest.TestCase, object):
    ...
    def test_file_and_domain_query(self):
        ...
        query = translation.translate(module, 'query', '{}', stix_pattern, options=test_options)
        queries = ["process_name:some_file.exe and domain:example.com"]
        _test_query_assertions(query, queries)
    ...
    def test_simple_and_query(self):
        ...
        query = translation.translate(module, 'query', '{}', stix_pattern, options=test_options)
        queries = ["process_name:cmd.exe and -(username:SYSTEM)"]
        _test_query_assertions(query, queries)
    ...
    def test_unmapped_attribute_handling_with_OR(self):
        ...
        translated_query = ["ipaddr:198.51.100.5"]
        result = translation.translate(module, 'query', '{}', stix_pattern, options=test_options)
    ...
    def test_binary_api_qualifier(self):
        stix_to_cb_mapping = {
                "[file:name = 'cmd.exe'] START t'2019-01-22T00:04:52.937Z' STOP t'2019-02-22T00:04:52.937Z']": ["(process_name:cmd.exe) and last_update:[2019-01-22T00:04:52 TO 2019-02-22T00:04:52]"],
                "[file:hashes.MD5 = '79054025255fb1a26e4bc422aef54eb4']": ["md5:79054025255fb1a26e4bc422aef54eb4"],
    ...
    def test_merge_apis(self):
        stix_to_cb_mapping = {
                "[process:name = 'cmd.exe']": ["process_name:cmd.exe"],
                "[process:name = 'cmd.exe' AND file:hashes.MD5 = 'blah']": ["process_name:cmd.exe and md5:blah"],
                "[process:name = 'cmd.exe' OR file:hashes.MD5 = 'blah']": ["process_name:cmd.exe or md5:blah"],
                "[file:hashes.MD5 = 'blah']": ["md5:blah"],
                "[process:name = 'cmd.exe'] OR [file:hashes.MD5 = 'blah']": ["(process_name:cmd.exe) or (md5:blah)"],
                "[process:name = 'cmd.exe'] OR [file:hashes.MD5 = 'blah'] OR [process:pid = 5]": ["((process_name:cmd.exe) or (md5:blah)) or (process_pid:5)"],
                }
        for stix_pattern, queries in stix_to_cb_mapping.items():
            test_options = {"time_range": None} 
            result = translation.translate("carbonblack", 'query', '{}', stix_pattern, options=test_options)
    ...
    def test_nested_parenthesis_in_pattern(self):
        ...
        query = translation.translate(module, 'query', '{}', stix_pattern, options=test_options)
        queries = ["((ipaddr:192.168.122.83 or ipaddr:100.100.122.90) and ipport:37020) or username:root"]
        _test_query_assertions(query, queries)

    def test_start_stop_merged(self):
        stix_to_cb_mapping = {
                "[process:name = 'cmd.exe'] OR [file:name = 'notepad.exe'] START t'2014-01-13T07:03:17Z' STOP t'2014-01-13T07:03:17Z'" : ["(process_name:cmd.exe) or ((process_name:notepad.exe) and last_update:[2014-01-13T07:03:17 TO 2014-01-13T07:03:17])"],
                "[process:name = 'cmd.exe'] START t'2014-01-13T07:03:17Z' STOP t'2019-01-13T07:03:17Z'  OR [file:name = 'notepad.exe'] START t'2014-01-13T07:03:17Z' STOP t'2014-01-13T07:03:17Z'": ["((process_name:cmd.exe) and last_update:[2014-01-13T07:03:17 TO 2019-01-13T07:03:17]) or ((process_name:notepad.exe) and last_update:[2014-01-13T07:03:17 TO 2014-01-13T07:03:17])"],
                "([process:name = 'cmd.exe'] OR [process:name = 'notepad.exe']) START t'2014-01-13T07:03:17Z' STOP t'2014-01-13T07:03:17Z'": ["((process_name:cmd.exe) and last_update:[2014-01-13T07:03:17 TO 2014-01-13T07:03:17]) or ((process_name:notepad.exe) and last_update:[2014-01-13T07:03:17 TO 2014-01-13T07:03:17])"],
                "([process:name = 'cmd.exe'] OR [file:name = 'notepad.exe']) START t'2014-01-13T07:03:17Z' STOP t'2014-01-13T07:03:17Z'" : ["((process_name:cmd.exe) and last_update:[2014-01-13T07:03:17 TO 2014-01-13T07:03:17]) or ((process_name:notepad.exe) and last_update:[2014-01-13T07:03:17 TO 2014-01-13T07:03:17])"],
                }
        for stix_pattern, queries in stix_to_cb_mapping.items():
            test_options = {"time_range": None}
            result = translation.translate("carbonblack", 'query', '{}', stix_pattern, options=test_options)
