...
class QueryStringPatternTranslator:
    ...
    def _parse_expression(self, expression) -> str:
        if isinstance(expression, ComparisonExpression):  # Base Case
            ...
            if isinstance(expression.expr1, StartStopQualifier):
                expression_01 = "({})".format(expression_01)
            if isinstance(expression.expr2, StartStopQualifier):
            ...
            return "{} AND {}".format(self._parse_expression(expression.observation_expression), _format_start_stop_qualifier(expression))
...
def _format_start_stop_qualifier(qualifier: StartStopQualifier) -> str:
    start_time = _test_or_add_milliseconds("'" + qualifier.start_iso + "'")
    stop_time = _test_or_add_milliseconds("'" + qualifier.stop_iso + "'")
    return '(@timestamp:["' + str(start_time) + '" TO "' + str(stop_time) + '"])'
