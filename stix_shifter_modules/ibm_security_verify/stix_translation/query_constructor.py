...
REFERENCE_DATA_TYPES = {
    "ipaddr": ["ipv4", "ipv4_cidr", "ipv6", "ipv6_cidr"],
...
class QueryStringPatternTranslator:
    ...
    def _parse_mapped_fields(
        ...
        for mapped_field in mapped_fields_array:
            if is_reference_value:
                ...
                comparison_string += (
                    'filter_key={mapped_field}&filter_value="{value}"'.format(
                        mapped_field=mapped_field, comparator=comparator, value=value
                    )
                )
            else:
    ...
    def _is_object_filterType(stix_object, stix_field):
        if (stix_object in REFERENCE_FILTER_TYPE) and (
            stix_field not in REFERENCE_EXCLUDE_FILTER_TYPE
        ):
            return True
        else:
            return False
        ...
        if isinstance(expression, ComparisonExpression):  # Base Case
            ...
            if (
                ...
                in_string = (
                    ...
                    if hasattr(expression.value, "values")
...
def query_validation(query: str):
    if not isinstance(query, str):
        raise TypeError("Input parameter is not a string!")
    query_keyword = ['filter_key','event_type']
    filter_key_count=0
    event_type_count = 0
    filter_key_count = query.count(query_keyword[0])
    event_type_count = query.count(query_keyword[1])
    print(filter_key_count)
    print(event_type_count)
    if(filter_key_count>1) :
        logging.error ('Operator \'And\' with multiple data object is not supported. please refer documentation')
        raise KeyError( 'Operator \'And\' with multiple data object is not supported. please refer documentation')
    if(event_type_count>1):
        raise KeyError( 'Operator \'And\' with multiple event type  is not supported. please refer documentation')
