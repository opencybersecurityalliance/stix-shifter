...
class TestStixToQuery(unittest.TestCase, object):
    def test_artifact_payload_query(self):
        ...
        queries = '{"query": "(_raw = \\"Sep 26 05:29:06 sumologic NetworkManager[677]: <info>  [1632614346.1076] dhcp4 (eth0)\\")",\n"fromTime": "%s",\n"toTime": "%s"}' % (from_time, to_time)
    ...
    def test_event_count_query(self):
        ...
        queries = "{\"query\": \"(_messagecount = \\\"21\\\")\",\n\"fromTime\": \"20210128T122401\"," \
    ...
    def test_time_observed_query(self):
        ...
        _, from_time, to_time = query_constructor.convert_timestamp(query)
        queries = "{\"query\": \"(_messagetime = \\\"2021-09-23T11:34:07.255Z\\\")\",\n\"fromTime\": \"%s\"," \
                  "\n\"toTime\": \"%s\"}" % (from_time, to_time)
        _test_query_assertions(query, queries)
    ...
    def test_src_device_query(self):
        ...
        query = translation.translate('sumologic', 'query', '{}', stix_pattern)
        _, from_time, to_time = query_constructor.convert_timestamp(query)
        queries = "{\"query\": \"(_collector = \\\"sumologic.domain.com\\\")\",\n\"fromTime\": \"%s\"," \
                  "\n\"toTime\": \"%s\"}" % (from_time, to_time)
        _test_query_assertions(query, queries)
    ...
    def test_event_code_query(self):
        ...
        query = translation.translate('sumologic', 'query', '{}', stix_pattern)
        _, from_time, to_time = query_constructor.convert_timestamp(query)
        queries = "{\"query\": \"(_messageid = \\\"12345678\\\")\",\n\"fromTime\": \"%s\",\n\"toTime\": \"%s\"}" % (
            from_time, to_time)
        _test_query_assertions(query, queries)
    ...
    def test_event_module_query(self):
        ...
        query = translation.translate('sumologic', 'query', '{}', stix_pattern)
        _, from_time, to_time = query_constructor.convert_timestamp(query)
        queries = "{\"query\": \"(_source = \\\"Linux System_3\\\")\",\n\"fromTime\": \"%s\",\n\"toTime\": \"%s\"}" % (
            from_time, to_time)
        _test_query_assertions(query, queries)
    ...
    def test_event_provider_query(self):
        ...
        query = translation.translate('sumologic', 'query', '{}', stix_pattern)
        _, from_time, to_time = query_constructor.convert_timestamp(query)
        queries = "{\"query\": \"(_sourcecategory = \\\"linux/system\\\")\",\n\"fromTime\": \"%s\"," \
                  "\n\"toTime\": \"%s\"}" % (from_time, to_time)
        _test_query_assertions(query, queries)
