...
class QueryTranslator(EmptyQueryTranslator):
    ...
    def transform_query(self, data, antlr_parsing_object={}):
        ...
        response = client.call_api('/transform_query', 'POST', data=json.dumps({'module': connection['type'], 'query': data, 'options': connection['options']}), timeout=self.options.get('timeout'))
