...
from stix_shifter_modules.threat_grid.entry_point import EntryPoint
from stix_shifter_modules.threat_grid.tests.stix_translation.json_translation import *

MODULE = "threat_grid"
DATA_SOURCE = {"type": "identity", "id": "identity--3532c56d-ea72-48be-a2ad-1a53f4c9c6d3", "name": "ThreatGrid_Connector",
...
class TestThreatGridResultsToStix(unittest.TestCase):
    """
    class to perform unit test case for translate results
    """

    def __init__(self,*args, **kwargs):
        super(TestThreatGridResultsToStix, self).__init__(*args, **kwargs)
        self.result_translator = entry_point.create_default_results_translator(dialect='default')
        self.result_bundle = self.result_translator.translate_results(data_source=DATA_SOURCE, data=threat_grid_transmitQueryData_ip_benign['data'])
        self.result_bundle_objects = self.result_bundle['objects']
        self.result_bundle = self.result_translator.translate_results(data_source=DATA_SOURCE, data=threat_grid_transmitQueryData_hash_mal['data'])
        self.result_bundle_objects_mal = self.result_bundle['objects']
        self.result_bundle = self.result_translator.translate_results(data_source=DATA_SOURCE, data=threat_grid_transmitQueryData_domain_none['data'])
        self.result_bundle_objects_none = self.result_bundle['objects']
        self.result_bundle = self.result_translator.translate_results(data_source=DATA_SOURCE, data=threat_grid_transmitQueryData_ip_anoma['data'])
    ...
    def exists(obj, chain):
        ...
        _key = chain.pop(0)
        if _key in obj:
            return TestThreatGridResultsToStix.exists(obj[_key], chain) if chain else obj[_key]
    ...
    def get_first_of_type(itr, typ):
        return TestThreatGridResultsToStix.get_first(itr, lambda o: type(o) == dict and o.get('type') == typ)
    ...
    def test_stix_identity_prop(self):
        ...
        """
        stix_identity = TestThreatGridResultsToStix.get_first_of_type(self.result_bundle_objects, DATA_SOURCE['type'])
    ...
    def test_stix_extension_prop(self):
        ...
        stix_extension = TestThreatGridResultsToStix.get_first_of_type(self.result_bundle_objects, sdo_type)
    ...
    def test_stix_indicator_prop(self):
        ...
        """
        sdo_type = 'indicator'
        stix_indicator = TestThreatGridResultsToStix.get_first_of_type(self.result_bundle_objects, sdo_type)
        assert 'type' in stix_indicator and stix_indicator['type'] == sdo_type
        assert 'pattern' in stix_indicator and stix_indicator['pattern'] == threat_grid_query_pattern
    ...
    def test_stix_indicator_prop_mal(self):
        ...
        """
        sdo_type = 'indicator'
        stix_indicator = TestThreatGridResultsToStix.get_first_of_type(self.result_bundle_objects_mal, sdo_type)
        assert 'type' in stix_indicator and stix_indicator['type'] == sdo_type
        assert 'pattern' in stix_indicator and stix_indicator['pattern'] == threat_grid_query_pattern_hash
        assert 'valid_from' in stix_indicator
        assert 'indicator_types' in stix_indicator and len(stix_indicator['indicator_types']) == 1 \
    ...
    def test_stix_indicator_prop_unknown(self):
        ...
        """
        sdo_type = 'indicator'
        stix_indicator = TestThreatGridResultsToStix.get_first_of_type(self.result_bundle_objects_none, sdo_type)
        assert 'type' in stix_indicator and stix_indicator['type'] == sdo_type
        assert 'pattern' in stix_indicator and stix_indicator['pattern'] == threat_grid_query_pattern_domain
        assert 'valid_from' in stix_indicator
        assert 'indicator_types' in stix_indicator and len(stix_indicator['indicator_types']) == 1 \
    ...
    def test_stix_indicator_prop_anomalous(self):
        ...
        """
        sdo_type = 'indicator'
        stix_indicator = TestThreatGridResultsToStix.get_first_of_type(self.result_bundle_objects_anomalous, sdo_type)
        assert 'type' in stix_indicator and stix_indicator['type'] == sdo_type
        assert 'pattern' in stix_indicator and stix_indicator['pattern'] == threat_grid_query_pattern_ip
        assert 'valid_from' in stix_indicator
        assert 'indicator_types' in stix_indicator and len(stix_indicator['indicator_types']) == 1 \
    ...
    def test_stix_indicator_extensions_prop(self):
        ...
        """
        stix_extension = TestThreatGridResultsToStix.get_first_of_type(self.result_bundle_objects, 'extension-definition')
        stix_indicator = TestThreatGridResultsToStix.get_first_of_type(self.result_bundle_objects, 'indicator')
        ...
        is_exist = TestThreatGridResultsToStix.exists(stix_indicator, property_name.split("."))
