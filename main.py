...
def __main__():
    ...
    if args.command == EXECUTE:
        ...
        for query in dsl['queries']:
            search_id = transmission.query(query)

            print("Executed search; id is {}".format(search_id))
            ...
            print("Search {} results is:\n{}".format(search_id,result))

            # Collect all results
            results += result

        # Translate results to STIX
        result = translation.translate(args.translation_module, 'results', args.data_source, json.dumps(result) )
        print( result )
