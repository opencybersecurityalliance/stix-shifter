...
def convert_to_stix(data_source, map_data, data, transformers, options, callback=None):
    ...
    ds2stix = DataSourceObjToStixObj(
        identity_id, map_data, transformers, options, callback)
    ...
    def _transform(self, object_map, observation, ds_map, ds_key, obj):
        ...
        if isinstance(to_map, dict):
            ...
            for key in to_map.keys():
                self._transform(object_map, observation,
                                ds_map[ds_key], key, to_map)
            return
        ...
        if isinstance(ds_key_def_obj, list):
            ...
            if self.callback:
                try:
                    generic_hash_key = self.callback(
                        obj, ds_key, ds_key_def_obj['key'], self.options)
                except(Exception):
                    return
        ...
        for ds_key_def in ds_key_def_list:
            ...
            transformer = self.transformers[ds_key_def['transformer']
                                            ] if 'transformer' in ds_key_def else None
            ...
            if ds_key_def.get('cybox', self.cybox_default):
                ...
                DataSourceObjToStixObj._handle_cybox_key_def(
                    key_to_add, observation, stix_value, object_map, object_name, group)
            else:
                stix_value = DataSourceObjToStixObj._get_value(
                    obj, ds_key, transformer)
                if not DataSourceObjToStixObj._valid_stix_value(self.properties, key_to_add, stix_value):
