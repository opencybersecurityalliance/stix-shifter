...
def _fetch_network_protocol_mapping():
    try:
        map_file = open(
            'stix_shifter/stix_translation/src/modules/carbonblackprocess/json/network_protocol_map.json').read()
...
class CbPQueryStringPatternTranslator:
    comparator_lookup = {
    ...
    def _format_set(values) -> str:
        ...
        return "({})".format('q='.join([CbPQueryStringPatternTranslator._escape_value(value) for value in gen]))

    @staticmethod
    def _format_match(value) -> str:
        raw = CbPQueryStringPatternTranslator._escape_value(value)
        if raw[0] == "^":
            raw = raw[1:]
        else:
            raw = ".*" + raw
    ...
    def _format_like(value) -> str:
        ...
        return CbPQueryStringPatternTranslator._escape_value(value)
...
def translate_pattern(pattern: Pattern, data_model_mapping, result_limit, timerange=None):
    translated_statements = CbPQueryStringPatternTranslator(pattern, data_model_mapping, result_limit)
    queries = []
